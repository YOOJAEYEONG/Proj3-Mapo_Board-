...
public class PagingUtil {
	...
	public static String pagingImg(int totalRecordCount,
		...
		if(intTemp <= totalPage) {
			...
			pagingStr += "	<img src='../images/paging3.gif'></a>";
			pagingStr += "&nbsp;";
	...
	*/	
		...
		while(blockCount<=blockPage && intTemp<=totalPage) {
			...
			if(intTemp==nowPage) {
				...
				pagingStr += ""
						...
						+ 	"<a href='#' class='page-link'>"+intTemp+"</a>"
						+ "</li>";
			}
			else {
				//현재페이지가 아닌경우
	...
	public static String pagingBS74(int totalRecordCount,
			int pageSize, int blockPage, int nowPage, String pageName) {
		...
		while(blockCount<=blockPage && intTemp<=totalPage)
		{
			...
			if(intTemp==nowPage) {
				...
				//현재페이지가 아닌경우
				pagingStr += "<li class='page-item'>"
						+ "<a href='"+pageName+"nowPage="+intTemp+"' class='page-link'>"+intTemp+"</a>"
								+ "</li>";
			}
			intTemp++;
			blockCount++;
